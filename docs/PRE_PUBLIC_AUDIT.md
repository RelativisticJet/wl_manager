...
- **F-L11** (re-capture screenshots `02-inline-editing.png` + `03-audit-trail.png`) — **re-evaluated 2026-05-18 against live container, no replacement needed.** The deferred finding was a defensive coupling note: "if F-L6/F-L7 curation runs, screenshots displaying those rows must be re-taken." On inspection: (a) screenshot `02-inline-editing.png` shows the `DR130_priv_escalation` editing view — `DR130` was NOT in F-L6 scope (F-L6 curated `DR310`/`DR71_users`/`DR520`/`DR630`; F-L7 curated `DR20`/`DR102`/`DR88`). The content shown in 02 (synthetic usernames like `ahernandez_dev`, `admin_asmith`, `svc_splunk5` against AD groups and ServiceNow-style ticket IDs) is unchanged. (b) Screenshot `03-audit-trail.png` shows audit events for `DR45_suspicious_login`, `DR610_vpn_anomaly`, `DR640_service_account_logon` — none in F-L6/F-L7 scope; the visible row value `user_row_4: r.thomas` references a now-removed-from-host-but-historically-emitted audit event, which is correct historical behavior (audit log is immutable record of past actions). (c) Re-capture against the current 2026-05-18 container produced visually-worse output (the dev-license trial entered its warning-banner state in the 12 days since the originals, adding a prominent orange `DEVLICENSE:COMMUNICATE.OLEH@GMAIL.COM` banner that wasn't in the originals; the email is the intentional D15 publisher identity but the highlighted presentation is noisier). Action: keep originals; do NOT publish replacements. F-L11 closed with verified-non-finding status.
