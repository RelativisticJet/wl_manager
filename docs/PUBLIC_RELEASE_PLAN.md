...
> Created: 2026-05-13
> Status: **Phase 0 — Foundation cleanup COMPLETE for all 15 rows.** Phase 1 in progress — **1.1, 1.2, 1.3, 1.4, 1.5 all closed 2026-05-17**. Next: **Phase 1.6** — first manual `workflow_dispatch` of `appinspect-api.yml`, read the two profile reports from the workflow log, append findings to `docs/APPINSPECT_FINDINGS.md`. The single biggest unknown remaining in Phase 1 (R1.1 — `wl_fim_watch.py` `interval=0` viability on Cloud) gets surfaced here; if estimated refactor >2 weeks, D7 escalation triggers.
> Updated: 2026-05-17 (Phase 1.5 closed: `.github/workflows/appinspect-api.yml` invokes `splunk/appinspect-api-action@v3.0.5` against the SPLUNK_DEV_* secrets, runs both `cloud` and `private_app` profiles with `if: always()` so both reports surface even when one errors. Action wrapper used because local `splunk-appinspect 4.2.0` CLI has no remote/submit subcommand and the hosted API's auth handshake is undocumented; asymmetry vs Phase 1.2's pip-install pattern documented in the workflow header. Earlier this date: Phase 1.4 closed (Splunk Dev secrets registered 17:42 UTC); Phase 1.4 re-scoped per D18 (skip Cloud Sandbox); RUNBOOKS.md gained "AppInspect API — Credential Setup". Phase 1.2 + 1.3 landed: appinspect.yml CLI workflow + APPINSPECT_FINDINGS.md baseline; app.manifest version drift fixed; RELEASE_CHECKLIST.md §3.5 pre-flight extended)
