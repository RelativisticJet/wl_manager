...
> Created: 2026-05-13
> Status: **Phase 0 — Foundation cleanup COMPLETE for all 15 rows (original 14 + Phase 0.12 added post-close and closed retroactively after discovery that R6-F8 was already shipped in builds 656-657)**
> Updated: 2026-05-15 (Phase 0 closed; post-close gap review added Phase 0.12 + Phase 2.15-2.18 + optional polish subsection. Phase 0.12 closed same-day after live grep showed the R6-F8 fix had already shipped — see footnote at end of §3 + Phase 0.12 row itself for the audit trail)
...
| # | Task | Acceptance | Est. |
|---|------|------------|------|
| 1.1 ⚠️ PARTIAL 2026-05-13 (per Decision D15 in §1; marked retroactively during 2026-05-15 gap review as G3; demoted from ✅ to ⚠️ PARTIAL on 2026-05-16 per QA-C5 finding) | Sign up for Splunk Developer account at dev.splunk.com. Account `Oleh Bezsonov` / `communicate.oleh@gmail.com` is active per D15. **Open sub-item (1.1.1):** the second half of the acceptance criterion — "credentials stored securely" — is not yet documented. Closure requires the user to record, in a private location, the credential-storage method (password manager / KMS / encrypted .env / 2FA-only-with-recovery-codes / etc) and add a one-line ack to `docs/DECISION_LOG.md` D15 that the storage method is documented out-of-repo. The credential itself MUST NOT be committed. Reversibility note per D15: transferable by emailing Splunk Developer Support if an LLC is ever formed. | Account active, credentials stored securely (method documented OUT-OF-REPO; only the ack lives in DECISION_LOG.md) | 15 min |
